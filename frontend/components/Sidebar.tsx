...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 min-h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2.5">
      ...
      <div className="px-5 py-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
        <button
          onClick={toggle}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800 transition-colors"
        >
          {theme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          {theme === "dark" ? "Light mode" : "Dark mode"}
        </button>
        <div className="text-xs text-slate-400 dark:text-slate-600">ACRP Prototype</div>
        <div className="text-xs text-slate-300 dark:text-slate-700">ENGR 408</div>
      </div>
    </aside>
  );
}
