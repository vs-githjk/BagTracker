...
export default function PassengerPage() {
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-white">Passenger Notification</h1>
        <p className="text-sm text-slate-400 mt-0.5">Look up bag status by passenger ID</p>
      </div>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-800 border border-slate-700 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
        <button
          onClick={() => lookup()}
          disabled={loading}
          className="px-4 py-2.5 text-sm bg-blue-700 hover:bg-blue-600 text-white rounded-lg font-medium transition-colors disabled:opacity-50"
        >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {demoIds.map((id) => (
          <button
            ...
            className="px-2 py-1 text-xs bg-slate-800 border border-slate-700 rounded text-slate-400 hover:text-slate-200 hover:bg-slate-700 font-mono transition-colors"
          >
            {id}
          </button>
        ))}
        <span className="text-xs text-slate-600">or any passenger ID from the dashboard</span>
      </div>

      {error && (
        <div className="p-4 bg-red-950 border border-red-800 rounded-lg text-red-300 text-sm">{error}</div>
      )}
      ...
      {status && cfg && (
        <div className="space-y-4">
          ...
          <div className={`rounded-2xl border p-6 ${cfg.bg}`}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-200 text-sm leading-relaxed">{status.message}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
            <h2 className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Bag Details</h2>
